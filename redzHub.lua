...
local TabRace = Window:MakeTab({"Race-V4", "moon"})
...
TabDiscord:AddDiscordInvite({
    ...
    Invite = "https://discord.gg/b7eNQQepAa",
})
...
local Section = TabFarm:AddSection({"Cày Cấp"})
...
-- ⚙️ CÀI ĐẶT MODE FARM
_G.FarmMode = _G.FarmMode or "Quest" -- Quest | Normal | Nearest
...
function FlyTo(cf)
	if not cf or not _G.AutoFarm then return end
	lastTargetCF = cf
	flying = true
	...
	flyThread = task.spawn(function()
		while flying and _G.AutoFarm and task.wait() do
			local player = game.Players.LocalPlayer
			local char = player.Character
			local hrp = char and char:FindFirstChild("HumanoidRootPart")
			if not hrp then continue end

			local bv, bg = EnsureFlySystem()
			if not bv or not bg then continue end
...
-- 🧩 TỰ HỒI BAY KHI NHÂN VẬT RESET / TRANSFORM
game.Players.LocalPlayer.CharacterAdded:Connect(function(char)
	task.spawn(function()
		repeat task.wait() until char:FindFirstChild("HumanoidRootPart")
		if _G.AutoFarm and lastTargetCF then
			task.wait(0.5)
			FlyTo(lastTargetCF)
		end
	end)
...
plr.CharacterAdded:Connect(function(char)
	task.spawn(function()
		...
		if _G.AutoFarm then
			task.wait(1)
			CheckQuest()
			if lastTargetCF then FlyTo(lastTargetCF) end
		end
	end)
end)
...
task.spawn(function()
	while task.wait(0.15) do
		pcall(function()
			...
			local questUI = plr.PlayerGui and plr.PlayerGui.Main and plr.PlayerGui.Main.Quest
			local hasQuest = questUI and questUI.Visible
			local questTitle = hasQuest and questUI.Container.QuestTitle.Title.Text or ""

			-- 🟢 QUEST MODE
			if _G.FarmMode == "Quest" then
				if not hasQuest or (NameMon and not string.find(questTitle, NameMon)) then
			...
			if EnemyFolder then
				if _G.FarmMode == "Nearest" then
					...
					for _,v in pairs(EnemyFolder:GetChildren()) do
						if v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 then
							if string.find(string.lower(v.Name), string.lower(Mon or "")) then
								Enemy = v
								break
							end
						end
			...
			-- 🧭 XỬ LÝ BAY + ĐÁNH
			if Enemy then
				FlyTo(CFrame.new(Enemy.HumanoidRootPart.Position))
				EquipWeapon(_G.SelectWeapon)
				AutoHaki()
				DoAttack()
			elseif CFrameMon then
				-- 💤 KHÔNG CÓ QUÁI: BAY LƠ LỬNG NGHỈ
				FlyTo(CFrameMon)
			end
		end)
	end
...
------------------------------------------------
-- 🏭 ĐÁNH NHÀ MÁY (TỰ BAY, NOCOLLIDE, HỒI SAU CHẾT)
------------------------------------------------
do
TabFarm:AddToggle({
    Name = "Đánh Nhà Máy",
                ...
                while _G.Factory do
                    pcall(function()
                        ...
                        if Core and Core:FindFirstChild("HumanoidRootPart") then
                            ...
                            flyspeed = 300
                            local target = Core.HumanoidRootPart

                            -- Bay đến Core cho đến khi đến gần hoặc tắt
                            repeat
                                if not _G.Factory then break end
                                ...
                                task.wait()
                            until (hrp.Position - target.Position).Magnitude <= 6 or not _G.Factory
...
------------------------------------------------
-- 🔁 HỒI LẠI KHI NHÂN VẬT CHẾT HOẶC BIẾN ĐỔI
------------------------------------------------
local plr = game.Players.LocalPlayer
plr.CharacterAdded:Connect(function(char)
    task.spawn(function()
        ...
        -- Nếu đang bật Đánh Nhà Máy thì tự bật lại
        if _G.Factory then
            task.wait(1)
            EnableNoclip(true)
            task.spawn(function()
                while _G.Factory do
                    pcall(function()
                        ...
                        if Core and Core:FindFirstChild("HumanoidRootPart") then
                            ...
                            local bv, bg = EnsureFlySystem()
                            flyspeed = 300
                            local pos = Core.HumanoidRootPart.Position + Vector3.new(0, 10, 0)
                            local dir = (pos - hrp.Position)
                            bv.Velocity = dir.Unit * flyspeed
                            bg.CFrame = CFrame.new(hrp.Position, pos)
                        end
...
local Section = TabFish:AddSection({"Câu Cá"})
...
TabFish:AddDropdown({
    Name = "Chọn Mồi Câu",
    Options = {"Basic Bait", "Kelp Bait", "Good Bait", "Abyssal Bait", "Fozen Bait", "Epic Bait", "Carnivore Bait"},
    Default = "Basic Bait",
    Callback = function(Value)
        -- Chưa có chức năng
    end
...
local Section = TabItem:AddSection({"Tộc"})
...
local Section = TabItem:AddSection({"Môn Võ"})

-- 🧩 TOGGLE
TabItem:AddToggle({
    Name = "Tự Lấy Võ Cá Mập",
    Default = false,
    Callback = function(Value)
        -- Chưa có chức năng
    end
...
-- 🧩 TOGGLE
TabItem:AddToggle({
    Name = "Tự Lấy Võ Thần Diệt Quyền",
    Default = false,
    Callback = function(Value)
        -- Chưa có chức năng
    end
...
-- 🧩 TOGGLE
TabItem:AddToggle({
    Name = "Tự Lấy Võ Huyết Quỷ Thuật",
    Default = false,
    Callback = function(Value)
        -- Chưa có chức năng
    end
...
local Section = TabItem:AddSection({"Vũ Khí"})

-- 🧩 TOGGLE
TabItem:AddToggle({
    Name = "Tự Lấy Kiếm Saw",
    Default = false,
    Callback = function(Value)
        -- Chưa có chức năng
    end
...
local Section = TabItem:AddSection({"Phụ Kiện"})

-- 🧩 TOGGLE
TabItem:AddToggle({
    Name = "Tự Lấy Áo Choàng Marine",
    Default = false,
    Callback = function(Value)
        -- Chưa có chức năng
    end
...
local Section = TabFruit:AddSection({"Raid Law"})

-- 🌀 HÀM BAY (Tween tốc độ 300)
function Tween(CF)
...
-- ⚙️ BODY GYRO + NOCLIP (hỗ trợ nhiều auto)
spawn(function()
	while task.wait() do
		pcall(function()
			...
			-- ✅ Danh sách các auto đang kích hoạt (rút gọn + tối ưu)
			local active =
				_G.TeleportIsland or _G.SailBoat or _G.Auto_Holy_Torch or _G.FindMirageIsland or
				_G.TeleportPly or _G.Tweenfruit or _G.AutoFishCrew or _G.AutoShark or
				_G.AutoCakeV2 or _G.AutoMysticIsland or _G.AutoQuestRace or _G.AutoBuyBoat or
				_G.dao or _G.AutoMirage or Auto_Law or _G.AutoAllBoss or _G.AutoSeaBeast or
				_G.BossRaid or _G.GrabChest or _G.AutoLevel or _G.AutoBone or _G.Factory or
				_G.AutoElite or _G.AutoKenHop
...
spawn(function()
	pcall(function()
		game:GetService("RunService").Stepped:Connect(function()
			local char = game.Players.LocalPlayer.Character
			if not char then return end
			if _G.TeleportIsland or _G.SailBoat or _G.Auto_Holy_Torch or _G.FindMirageIsland or _G.TeleportPly or Auto_Law then
...
TabFruit:AddToggle({
    ...
    Callback = function(Value)
        ...
        if Value then
            ...
            task.spawn(function()
                pcall(function()
                    while _G.Auto_Law and task.wait(0.3) do
                        local plr = game:GetService("Players").LocalPlayer
                        local char = plr.Character or plr.CharacterAdded:Wait()

                        local hasChip =
                            char:FindFirstChild("Microchip") or
                            plr.Backpack:FindFirstChild("Microchip")

                        local hasOrder =
                            workspace.Enemies:FindFirstChild("Order") or
                            game.ReplicatedStorage:FindFirstChild("Order")

                        if not hasChip and not hasOrder then
                            local args = { "BlackbeardReward", "Microchip", "1" }
                            game.ReplicatedStorage.Remotes.CommF_:InvokeServer(unpack(args))
                            local args2 = { "BlackbeardReward", "Microchip", "2" }
                            game.ReplicatedStorage.Remotes.CommF_:InvokeServer(unpack(args2))
                        end
                    end
                end)
            end)

            -- ⚔️ Auto Summon + Kill Boss Law
            task.spawn(function()
                pcall(function()
                    while _G.Auto_Law and task.wait(0.4) do
                        local plr = game:GetService("Players").LocalPlayer
                        local char = plr.Character
                        local hasChip =
                            char:FindFirstChild("Microchip") or
                        ...
                        local hasOrderRep = game.ReplicatedStorage:FindFirstChild("Order")
                        local hasOrderWork = workspace.Enemies:FindFirstChild("Order")

                        -- Summon nếu có chip mà chưa spawn boss
                        if not hasOrderWork and not hasOrderRep and hasChip then
                            fireclickdetector(workspace.Map.CircleIsland.RaidSummon.Button.Main.ClickDetector)
                        end

                        -- Đã có boss thì bay đến đánh
                        if hasOrderRep or hasOrderWork then
                            if hasOrderWork then
                                    ...
                                    if v.Name == "Order" and v:FindFirstChild("HumanoidRootPart") then
                                        repeat task.wait(_G.Fast_Delay)
                                            pcall(function()
                                                ...
                                                Tween(v.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0)) -- bay trên boss
                            ...
                            elseif hasOrderRep then
                                Tween(CFrame.new(-6217.2021, 28.0476, -5053.1357)) -- bay đến nút summon
                            end
                        end
                    end
                end)
...
local Section = TabServer:AddSection({"Máy chủ Vip"})

-- 🧩 BUTTON
TabServer:AddButton({
    Name = "Nhận Server Vip Free",
    Callback = function()
        -- Chưa có chức năng
    end
})
...
local Section = TabServer:AddSection({"Máy Chủ Thường"})
...
local Section = TabServer:AddSection({"Tìm/Đổi Máy Chủ"})

do
-- 🌕 FIND FULL MOON (Toggle kiểu ESP Player)
TabServer:AddToggle({
    Name = "Tự Tìm Trăng Tròn",
    Default = false,
    Callback = function(Value)
        ...
        if Value then
            -- ✅ Khi bật
            task.spawn(function()
                while _G.AutoFindMoon and task.wait(2) do
                    local moonId = game:GetService("Lighting").Sky.MoonTextureId
                    if moonId == "http://www.roblox.com/asset/?id=9709149052" 
                    or moonId == "http://www.roblox.com/asset/?id=9709149431" then
                        -- 🌕 Full Moon → không hop
                        wait(2)
                    elseif moonId == "http://www.roblox.com/asset/?id=9709143733"
                    or moonId == "http://www.roblox.com/asset/?id=9709150401"
                    or moonId == "http://www.roblox.com/asset/?id=9709149680" then
                        -- 🌙 Các giai đoạn khác → hop server
                        Hop()
                    else
                        -- 🔄 Không rõ trạng thái → hop dự phòng
                        Hop()
                    end
                end
            end)
...
-- 🌫️ FIND MIRAGE ISLAND + BODY GYRO + NOCLIP
do
TabServer:AddToggle({
    Name = "Tự Tìm Đảo Kì Bí",
    Default = false,
    Callback = function(Value)
        _G.FindMirageIsland = Value
        _G.BodyGyroEnable = Value -- tự bật/tắt body gyro cùng toggle

        if Value then
            -- ✅ BodyClip + NoClip song song
            task.spawn(function()
                local RunService = game:GetService("RunService")

                -- BodyVelocity giữ ổn định
                task.spawn(function()
                    while _G.BodyGyroEnable and task.wait() do
                        pcall(function()
                            ...
                            local active = _G.FindMirageIsland
                            if active then
                                if not hrp:FindFirstChild("BodyClip") then
                                    local clip = Instance.new("BodyVelocity")
                                    clip.Name = "BodyClip"
                ...
                RunService.Stepped:Connect(function()
                    if not _G.BodyGyroEnable then return end
                    local char = game:GetService("Players").LocalPlayer.Character
                    if not char then return end
                    for _, v in pairs(char:GetDescendants()) do
                        if v:IsA("BasePart") then
            ...
            -- ✅ Find Mirage Island
            task.spawn(function()
                while _G.FindMirageIsland and task.wait(3) do
                    local map = game:GetService("Workspace"):FindFirstChild("Map")
                    if not map then continue end

                    local found = map:FindFirstChild("MysticIsland")
                    if found then
...
local Section = TabStats:AddSection({"Thông Số"})
...
local Section = TabStats:AddSection({"Tăng Chỉ Số"})
...
local Section = TabTeleport:AddSection({"Khu Vực"})

------------------------------------------------
-- 🌍 TELEPORT TAB (Auto Detect World by PlaceId) [Version 2]
------------------------------------------------
local placeId2 = game.PlaceId
local World1_2 = placeId2 == 2753915549
local World2_2 = placeId2 == 4442272183
local World3_2 = placeId2 == 7449423635

if World1_2 then
	TabTeleport:AddDropdown({
		Name = "Chọn Khu Vực",
		Options = {
			"Sky Island 2", "Sky Island 3", "Under Water Island"
		},
		Callback = function(v)
			_G.SelectIsland2 = v
		end
	})
elseif World2_2 then
	TabTeleport:AddDropdown({
		Name = "Chọn Khu Vực",
		Options = {
			"Flamingo Mansion", "Flamingo Room", "Cursed Ship"
		},
		Callback = function(v)
			_G.SelectIsland2 = v
		end
	})
elseif World3_2 then
	TabTeleport:AddDropdown({
		Name = "Chọn Khu Vực",
		Options = {
			"Mansion", "Castle On The Sea", "Time Of Temple", "Hydra Island", "Gallant Pirate", "Tiki Outpost"
		},
		Callback = function(v)
			_G.SelectIsland2 = v
		end
...
------------------------------------------------
-- ⚙️ HÀM BAY ĐẾN VỊ TRÍ (lơ lửng, tốc độ 300) [Version 2]
------------------------------------------------
...
function topos2(cf)
	pcall(function()
		...
		if not char2 or not char2:FindFirstChild("HumanoidRootPart") then return end
		local hrp2 = char2.HumanoidRootPart

		if currentTween2 then
			currentTween2:Cancel()
			currentTween2 = nil
		end

		-- Thêm hiệu ứng bay lơ lửng
		local targetCF2 = CFrame.new(cf.Position.X, cf.Position.Y + 3, cf.Position.Z)
		local distance2 = (hrp2.Position - targetCF2.Position).Magnitude
		local tweenInfo2 = TweenInfo.new(distance2 / 300, Enum.EasingStyle.Linear)

		currentTween2 = TweenService2:Create(hrp2, tweenInfo2, {CFrame = targetCF2})
		currentTween2:Play()

		currentTween2.Completed:Connect(function()
			currentTween2 = nil
		end)
	end)
end

function StopTween2(flag2)
	if not flag2 and currentTween2 then
		currentTween2:Cancel()
		currentTween2 = nil
	end
end
...
------------------------------------------------
-- 🚀 TOGGLE TELEPORT ĐẾN ĐẢO (Bay lơ lửng, Tốc độ 300) [Version 2]
------------------------------------------------
TabTeleport:AddToggle({
	Name = "Dịch Chuyển Đến Đảo",
	Default = false,
	Callback = function(state2)
		_G.TeleportIsland2 = state2

		if state2 then
			task.spawn(function()
				while _G.TeleportIsland2 do
					task.wait(0.25)

					local v2 = _G.SelectIsland2
					...
					local cf2 = nil
					-- Sea 1
					if v2 == "Sky Island 2" then
						game.ReplicatedStorage.Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(-4607.8227, 872.5424, -1667.5568))
					elseif v2 == "Sky Island 3" then
						game.ReplicatedStorage.Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(-7894.6176, 5547.1416, -380.2911))
					elseif v2 == "Under Water Island" then
						game.ReplicatedStorage.Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(61163.8515, 11.6796, 1819.7841))
					-- Sea 2
					elseif v2 == "Flamingo Mansion" then
						game.ReplicatedStorage.Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(-286.99, 306.17, 597.89))
					elseif v2 == "Flamingo Room" then
						game.ReplicatedStorage.Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(2284.4140, 15.1520, 875.7253))
					elseif v2 == "Cursed Ship" then
						game.ReplicatedStorage.Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(923.4019, 125.0571, 32885.875))
					-- Sea 3
					elseif v2 == "Mansion" then
						game.ReplicatedStorage.Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(-12471.1699, 374.9402, -7551.6777))
               elseif v2 == "Castle On The Sea" then
						game.ReplicatedStorage.Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(-5054.33, 314.56, -177.36))
               elseif v2 == "Time Of Temple" then
						game.ReplicatedStorage.Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(28286.35546875, 14895.3017578125, 102.62469482421875))
               elseif v2 == "Hydra Island" then
						game.ReplicatedStorage.Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(5660.99, 1013.08, -336.86))
               elseif v2 == "Gallant Pirate" then
						game.ReplicatedStorage.Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(5369.02, 25.23, -497.11))
               elseif v2 == "Tiki Outpost" then
						game.ReplicatedStorage.Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(-16813.44, 58.30, 304.87))
					end

					if cf2 then topos2(cf2) end
				end
			end)
		else
			StopTween2(_G.TeleportIsland2)
		end
	end
})
...
local Section = TabVisual:AddSection({"Troll"})

-- 🍈 RAIN FRUIT (FAKE)
TabVisual:AddButton({
    Name = "Rain Fruit (Old)",
    Callback = function()
        local fruits = game:GetObjects("rbxassetid://15970729030")[1]
        for _, v in pairs(fruits:GetChildren()) do
            ...
            if v:FindFirstChild("Fruit") and v.Fruit:FindFirstChild("AnimationController") then
                local anim = v.Fruit.AnimationController:LoadAnimation(v.Fruit.Idle)
                anim:Play()
            end

            if v:FindFirstChild("Handle") then
                v.Handle.Touched:Connect(function(otherPart)
                    if otherPart and otherPart.Parent == game.Players.LocalPlayer.Character then
...
local Section = TabVisual:AddSection({"Giao Diện"})
...
TabVisual:AddToggle({
    ...
    Callback = function(Value)
        ...
        local drawingsESP = {}
        local espConnection, recoveryLoop

        local ESP_COLOR = Color3.fromRGB(255, 140, 0) -- 🧡 Màu cam
        ...
        local function createESP(obj)
            ...
            if drawingsESP[obj] then return end
            ...
            drawingsESP[obj] = {text = text, part = part, name = obj.Name}
        end

        -- 🧹 Xoá ESP
        local function removeESP(obj)
            local data = drawingsESP[obj]
            if data then
                pcall(function()
        ...
        -- 🔍 Quét toàn bộ workspace để tìm object có tên chứa “Berr”
        local function scanBerries()
            for _, obj in pairs(workspace:GetDescendants()) do
                local name = string.lower(obj.Name)
                if string.find(name, "berr") then
                    createESP(obj)
                end
            end
        end
        ...
        local function startESP()
            ...
            espConnection = RunService.RenderStepped:Connect(function()
                for obj, data in pairs(drawingsESP) do
            ...
            -- Cập nhật lại liên tục
            recoveryLoop = task.spawn(function()
                while task.wait(2) and Value do
                    for _, obj in pairs(workspace:GetDescendants()) do
                        local name = string.lower(obj.Name)
                        if string.find(name, "berr") and not drawingsESP[obj] then
                            createESP(obj)
                        end
                    end
                    for obj in pairs(drawingsESP) do
        ...
        local function stopESP()
            ...
            espConnection, recoveryLoop = nil, nil
        end
...
------------------------------------------------
-- 🍈 ESP TRÁI ÁC QUỶ (Devil Fruit)
------------------------------------------------
function UpdateDevilChams()
	for i, v in pairs(workspace:GetChildren()) do
		pcall(function()
			if DevilFruitESP and v:FindFirstChild("Handle") and string.find(v.Name, "Fruit") then
				...
				local dist = round((head.Position - handle.Position).Magnitude / 3)
				local textDisplay = v.Name .. string.format(" [%dm]", dist)
				...
				if not handle:FindFirstChild(espName) then
					...
					name.Text = textDisplay
				else
					handle[espName].TextLabel.Text = textDisplay
				end
			elseif v:FindFirstChild("Handle") and v.Handle:FindFirstChild("NameEsp" .. Number) then
...
------------------------------------------------
-- 🍎 ESP TRÁI THẬT (Apple / Pineapple / Banana)
------------------------------------------------
function UpdateRealFruitChams()
	...
	local function AddFruitESP(spawner, color)
		for _, v in pairs(spawner:GetChildren()) do
			if v:IsA("Tool") and v:FindFirstChild("Handle") then
				...
				elseif handle:FindFirstChild(espName) then
	...
	if workspace:FindFirstChild("AppleSpawner") then
		AddFruitESP(workspace.AppleSpawner, Color3.fromRGB(255, 0, 0))
	end
	if workspace:FindFirstChild("PineappleSpawner") then
		AddFruitESP(workspace.PineappleSpawner, Color3.fromRGB(255, 174, 0))
	end
	if workspace:FindFirstChild("BananaSpawner") then
		AddFruitESP(workspace.BananaSpawner, Color3.fromRGB(255, 255, 0))
	end
end
...
-- 🌍 ESP ISLAND (Drawing-based, hiển thị Tên Đảo [xxxm])
TabVisual:AddToggle({
    Name = "Esp Island",
    Default = false,
    Callback = function(Value)
        ...
        IslandESP = Value
        -- 🔁 Giữ bảng và connection bên ngoài để không reset mỗi lần bật/tắt
        if not _G.IslandESP_Data then
            _G.IslandESP_Data = {
                drawings = {},
                loop = nil
            }
        ...
        local drawingsESP = _G.IslandESP_Data.drawings
        local espLoop = _G.IslandESP_Data.loop

        -- 🌈 Cấu hình
        local COLOR_ISLAND = Color3.fromRGB(7, 236, 240)
        local TEXT_SIZE = 15
        local TEXT_FONT = 2
        ...
        local function createESP(obj)
            ...
            text.Visible = false
            text.Color = COLOR_ISLAND
            text.Font = TEXT_FONT
            drawingsESP[obj] = {text = text, part = obj}
        end
        ...
        local function updateIslandList()
            ...
            local locations = folder and folder:FindFirstChild("Locations")
            if not locations then return end

            for _, island in pairs(locations:GetChildren()) do
                if island.Name ~= "Sea" and not drawingsESP[island] then
                    createESP(island)
                end
            end

            for island in pairs(drawingsESP) do
        ...
        -- 🚀 Bắt đầu ESP
        local function startESP()
            if _G.IslandESP_Data.loop then return end
            _G.IslandESP_Data.loop = RunService.RenderStepped:Connect(function()
                if not IslandESP then return end

                updateIslandList()

                for island, data in pairs(drawingsESP) do
                    local part = data.part
                    if part and part:IsDescendantOf(workspace) and Player.Character and Player.Character:FindFirstChild("Head") then
                        local pos, onScreen = Camera:WorldToViewportPoint(part.Position + Vector3.new(0, 2, 0))
                        if onScreen then
                            local dist = (part.Position - Player.Character.Head.Position).Magnitude / 3
                            data.text.Text = string.format("%s [%.0fm]", part.Name, dist)
                            data.text.Position = Vector2.new(pos.X, pos.Y)
        ...
        -- ⛔ Dừng ESP (ẩn và xoá toàn bộ)
        local function stopESP()
            if _G.IslandESP_Data.loop then
                _G.IslandESP_Data.loop:Disconnect()
                _G.IslandESP_Data.loop = nil
            end
            for _, data in pairs(drawingsESP) do
                pcall(function()
                    if data.text then data.text:Remove() end
...
TabVisual:AddToggle({
    ...
    Callback = function(Value)
        ...
        -- 🎨 Màu
        local COLOR_BLUE = Color3.fromRGB(0, 85, 255)
        local COLOR_RED = Color3.fromRGB(255, 0, 0)

        -- 🪷 Danh sách tên thật của hoa trong game
        local FLOWER_NAMES = {
            ["Flower1"] = {DisplayName = "Blue Flower", Color = COLOR_BLUE},
            ["Flower2"] = {DisplayName = "Red Flower", Color = COLOR_RED}
        }

        -- 🧩 Tạo ESP cho hoa
        local function createESP(obj, nameInfo)
            if not obj:IsA("Model") then return end
            ...
            text.Outline = true
            text.Visible = false
            text.Color = nameInfo.Color

            drawingsESP[obj] = {text = text, part = part, name = nameInfo.DisplayName}
        end
        ...
        -- 🔍 Quét tất cả hoa hiện có
        local function scanFlowers()
            for _, obj in pairs(workspace:GetDescendants()) do
                if FLOWER_NAMES[obj.Name] then
                    createESP(obj, FLOWER_NAMES[obj.Name])
                end
            end
        end
        ...
        local function startESP()
            ...
            -- Kiểm tra lại hoa mỗi vài giây
            recoveryLoop = task.spawn(function()
                while task.wait(1) and Value do
                    for _, obj in pairs(workspace:GetDescendants()) do
                        if FLOWER_NAMES[obj.Name] and not drawingsESP[obj] then
                            createESP(obj, FLOWER_NAMES[obj.Name])
                        end
                    end
                    for obj in pairs(drawingsESP) do
...
------------------------------------------------
-- 🌫️ ESP MIRAGE ISLAND [Fixed Version]
------------------------------------------------
...
TabVisual:AddToggle({
	...
	Callback = function(Value)
			...
			for _, v in pairs(locations:GetChildren()) do
				pcall(function()
					if MirageIslandESP and v.Name == "Mirage Island" then
						...
						local dist = round((head.Position - v.Position).Magnitude / 3)
						local textDisplay = string.format("%s [%dm]", v.Name, dist)
						...
						if not v:FindFirstChild(espName) then
							...
							v[espName].TextLabel.Text = textDisplay
						end
					end
				end)
			end
		...
		-- 🧹 Hàm xoá toàn bộ Billboard khi tắt
		local function ClearAllMirageESP()
			local folder = workspace:FindFirstChild("_WorldOrigin")
			local locations = folder and folder:FindFirstChild("Locations")
			if not locations then return end
			for _, v in pairs(locations:GetChildren()) do
		...
		-- 🔁 Kiểm soát vòng lặp update
		if not _G.MirageESP_Thread then
			_G.MirageESP_Thread = nil
		end

		-- ✅ Bật ESP
		if Value then
			_G.MirageESP_Thread = task.spawn(function()
...
------------------------------------------------
-- ⚙️ ESP GEAR (Mystic Island) [Fixed Version]
------------------------------------------------
...
TabVisual:AddToggle({
	...
	Callback = function(Value)
		...
		-- 🔁 Giữ vòng lặp bên ngoài toggle
		if not _G.GearESP_Connection then
			_G.GearESP_Connection = nil
		end
		...
		-- ✅ Bật ESP
		if Value then
			-- Tạo vòng lặp update mới
			_G.GearESP_Connection = task.spawn(function()
				while GearESP do
					UpdateGearESP()
					task.wait(0.5)
				end
...
TabShop:AddToggle({
    ...
    Callback = function(Value)
        ...
        if Value then
            task.spawn(function()
                while _G.AutoRandomBone and task.wait(0.2) do
                    pcall(function()
...
TabShop:AddButton({
    Name = "Mua Water Kung Fu [ $750,000 ]",
    Callback = function()
...
TabShop:AddButton({
    Name = "Mua Death Step [ ƒ5,000 $5,000,000 ]",
    Callback = function()
...
TabShop:AddButton({
    Name = "Mua Sharkman Karate [ ƒ5,000 $2,500,000 ]",
    Callback = function()
        local Replicated = game:GetService("ReplicatedStorage")
...
TabShop:AddButton({
    Name = "Mua Electric Claw [ ƒ5,000 $3,000,000 ]",
    Callback = function()
...
TabShop:AddButton({
    Name = "Mua Dragon Talon [ ƒ5,000 $3,000,000 ]",
    Callback = function()
...
TabShop:AddButton({
    Name = "Mua God Human [ ƒ5,000 $5,000,000 ]",
    Callback = function()
...
TabShop:AddButton({
    Name = "Mua Sanguine Art [ ƒ5,000 $5,000,000 ]",
    Callback = function()
        local Replicated = game:GetService("ReplicatedStorage")
...
TabShop:AddButton({
    Name = "Mua Dual-Headed Blade [ $400,000 ]",
    Callback = function()
...
TabShop:AddButton({
    Name = "Mua Refined Slingshot [ $30,000 ]",
    Callback = function()
...
TabShop:AddButton({
    Name = "Mua Refined Flintlock [ $65,000 ]",
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyItem", "Refined Flintlock")
    end
})
...
TabShop:AddButton({
    Name = "Mua Bizarre Rifle [ 250 Ectoplasm ]",
    Callback = function()
        local Replicated = game:GetService("ReplicatedStorage")
...
TabShop:AddButton({
    Name = "Mua Swordsman Hat [ $150,000 ]",
    Callback = function()
